...
$\mu = \mu_0 = 4\pi \times 10^{-7} \text{Hm}^{-1}$. \cite[Ch.4.1-4.4]{russenschuck_field_2011}
...
we can start to eliminate the impossible ones. In the end, the scalar
potential $\Psi(r, \varphi, z)$ will consist of a Bessel series in $r$,
a Fourier series in $\varphi$ and a Fourier series in $z$. A fitting
name is then the 'Bessel-Fourier-Fourier series', or BFF for short.
...
Now, all the potential solutions to $\Psi(r,\varphi,z) = Z(z)R(r)\Phi(\varphi)$ 
can be combined into one double
series.
By converting the two Fourier' series from real to complex
...
After a lot of algebra, this yields the solution
...
can be merged into one summand, they are however separated
to make the distinction of the fundamental fourier series
in z and the higher order harmonics clearer.
The individual summands are hereafter given names for reference
and can be seen in table \ref{tab:BFF-components}.
Using the notation $\Psi_{n,k}$
for the different harmonics, we have
...
\begin{figure}[!h]
    ...
    \caption{$B_z$ field along the z axis.}
