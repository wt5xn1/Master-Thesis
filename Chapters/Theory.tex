...
\begin{equation}
    ...
    \label{eq:amperelaw}
\end{equation}
where the surface integral of the current density has been replaced
as the number of turns $N$ times the enclosed current $I$. $S$ is a surface enclosing
the current density $\vb{J}$ and $C$ is the boundary of $S$. $\vb{l}$
is parametrized line describing C.
...
A curve $\vb{l} = \vb{l_1} + \vb{l_2} + \vb{l_3} + \vb{l_4}$ encloses
one end of a simple current loop. The axis of the current loop
has some angle $-\pi < \theta < \pi$ relative to the coordinate system.
If $l_1$ is sufficiently long, such that it's ends are far out in
the zero field, then the field at the remaining lines $\vb{l}_{2,3,4}$ will also be
very close to zero. A good approximation of equation \ref{eq:amperelaw} is
then
...
surrounding the domain. The dipoles in $r$ and $\varphi$ do
depend on the angle $\theta$. This relationship can not easily
be found algebraically, and will be studied more in chapter \ref{ch:xydipoles}.